...
def speak_out(predicted_class):
    class_label = decode_predictions(predicted_class)
    tts = gTTS(text=f"The predicted class is {class_label}", lang='en')
...
def predict():
    ...
    # Ensure the predicted_class and confidence are in the correct format
    # predicted_class_str = str(predicted_class)
    # confidence_float = float(confidence)


    return jsonify({'predicted_class': predicted_class, 'confidence': float(confidence)})
    # return render_template('production.html', data = f"predicted_classfloat(confidence)"})
...
def prediction():
    im_path = app.config['UPLOAD_FOLDER'] + "img.jpg"  # Path to save uploaded image

    img = request.files['img']
    img.save(im_path)
    color = cv2.imread(im_path, 1)
    resized_color = cv2.resize(color, (64, 64))

    confidence_threshold = 0.6
    resized_rgb = cv2.cvtColor(resized_color, cv2.COLOR_BGR2RGB)  # Convert image to RGB format
    resized_rgb_normalized = resized_rgb / 255.0  # Normalize the image
    preprocessed_image = np.expand_dims(resized_rgb_normalized, axis=0)
